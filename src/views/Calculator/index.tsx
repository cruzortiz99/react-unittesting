...
export function Calcultator(props: CalculatorProps): JSX.Element {
  const [firstNumber, _setFirstNumber] = useState(0);
  const [secondNumber, _setSecondNumber] = useState(0);
  const [result, setResult] = useState(0)
  const [errorMessage, setErrorMessage] = useState('')
  const [operationSelected, _setOperationSelected] = useState(
    CalculatorOperation.ADD
  );
  const handleResultCalculation = useCallback((firstNumber: number, secondNumber:number, operation: CalculatorOperation) => {
    ...
    (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
      {
        ...
        handleResultCalculation(Number(event.target.value), secondNumber, operationSelected)
      },
    ...
    (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
      {
        ...
        handleResultCalculation(firstNumber, Number(event.target.value), operationSelected)
      },
